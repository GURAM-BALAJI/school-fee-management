...
if ($req_per == 1) {
	if (isset($_POST['add'])) {
		...
		$conn = $pdo->open();
		try {
			$stmt = $conn->prepare("SELECT COUNT(*) AS numrows FROM students WHERE students_regestration_no=:students_regestration_no");
