            ...
            <table id="dataTable" class="table table-striped">
                ...
                <tbody>
                    ...
                    try {
                        ...
                        if ($classes_and_fee_value != 0 &&  $students_gender == 3)
                            $stmt = $conn->prepare("SELECT * FROM students WHERE students_class='$classes_and_fee_value' AND students_deleted='0' AND students_school_id=" . $_SESSION['admin_school_id'] . "");
                        if ($classes_and_fee_value == 0 &&  $students_gender != 3)
                            $stmt = $conn->prepare("SELECT * FROM students WHERE students_gender='$students_gender' AND students_deleted='0' AND students_school_id=" . $_SESSION['admin_school_id'] . "");
                        if ($classes_and_fee_value != 0 &&  $students_gender != 3)
                            $stmt = $conn->prepare("SELECT * FROM students WHERE students_gender='$students_gender' AND students_class='$classes_and_fee_value' AND students_deleted='0' AND students_school_id=" . $_SESSION['admin_school_id'] . "");
                        $stmt->execute();
                        $slno = 1;
                        foreach ($stmt as $row) {
                            echo "<tr>";
